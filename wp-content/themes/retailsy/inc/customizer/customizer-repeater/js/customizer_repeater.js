/* global jQuery */
/* global wp */
function retailsy_media_upload(button_class) {
    'use strict';
 ...
 *** Generate unique id ***
 *********************************************/
function retailsy_customizer_repeater_uniqid(prefix, more_entropy) {
    'use strict';
 ...
 *** General Repeater ***
 *********************************************/
function retailsy_customizer_repeater_refresh_social_icons(th) {
    'use strict';
    ...
    th.find('.customizer-repeater-social-repeater-container').each(function () {
        ...
        if (!id) {
            id = 'customizer-repeater-social-repeater-' + retailsy_customizer_repeater_uniqid();
    ...
    retailsy_customizer_repeater_refresh_general_control_values();
}


function retailsy_customizer_repeater_refresh_general_control_values() {
    'use strict';
    jQuery('.customizer-repeater-general-control-repeater').each(function () {
        ...
        th.find('.customizer-repeater-general-control-repeater-container').each(function () {
            ...
            if (!id) {
                id = 'social-repeater-' + retailsy_customizer_repeater_uniqid();
            ...
            if (text !== '' || image_url !== '' || image_url2 !== '' || title !== '' || subtitle !== '' || icon_value !== '' || link !== '' || choice !== '' || social_repeater !== '' || shortcode !== '' || slide_align !== '' || color !== '') {
                values.push({
                    ...
                    'color': color,
                    'color2': color2,
                    'text': retailsyescapeHtml(text),
                    'link': link,
                    'text2': retailsyescapeHtml(text2),
					'button_second': retailsyescapeHtml(button_second),
                    'link2': link2,
                    'image_url': (choice === 'customizer_repeater_none' ? '' : image_url),
                    'choice': choice,
                    'image_url2': image_url2,
                    'title': retailsyescapeHtml(title),
                    'subtitle': retailsyescapeHtml(subtitle),
					'subtitle2': retailsyescapeHtml(subtitle2),
					'subtitle3': retailsyescapeHtml(subtitle3),
					'subtitle4': retailsyescapeHtml(subtitle4),
					'subtitle5': retailsyescapeHtml(subtitle5),
					'slide_align': retailsyescapeHtml(slide_align),
					'open_new_tab' : open_new_tab,
                    'social_repeater': retailsyescapeHtml(social_repeater),
                    'id': id,
                    'shortcode': retailsyescapeHtml(shortcode)
                });
            }
...
jQuery(document).ready(function () {
    'use strict';
    var retailsy_theme_controls = jQuery('#customize-theme-controls');
    retailsy_theme_controls.on('click', '.customizer-repeater-customize-control-title', function () {
    ...
    retailsy_theme_controls.on('change', '.icp',function(){
        retailsy_customizer_repeater_refresh_general_control_values();
        return false;
    });
	
	retailsy_theme_controls.on('change','.customizer-repeater-slide-align', function(){
		retailsy_customizer_repeater_refresh_general_control_values();
        return false;
		
	});

    retailsy_theme_controls.on('change','.customizer-repeater-image2-control', function(){
        retailsy_customizer_repeater_refresh_general_control_values();
        return false;
        
    });
	
    retailsy_theme_controls.on('change', '.customizer-repeater-image-choice', function () {
    ...
    retailsy_media_upload('.customizer-repeater-custom-media-button');
     ...
     *
     */
    retailsy_theme_controls.on('click', '.customizer-repeater-new-field', function () {
        var split_add_more_button=jQuery(this).text();
        var split_add_more_button_split=split_add_more_button.substr(4, 12);
        var th = jQuery(this).parent();
        var id = 'customizer-repeater-' + retailsy_customizer_repeater_uniqid();
		
		if(split_add_more_button_split=="Add New Slid")
        {
        if(jQuery('#exist_retailsy_Slider').val()>=3)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_retailsy_Slider').val()<3)
        {
         var new_service_add_val=parseInt(jQuery('#exist_retailsy_Slider').val())+1;
         jQuery('#exist_retailsy_Slider').val(new_service_add_val);  
        }
        }
		
		 if(split_add_more_button_split=="Add New Info")
        {
        if(jQuery('#exist_retailsy_Information').val()>=4)
        {
        jQuery(".customizer_information_section_premium").show();
        return false;   
        }
         if(jQuery('#exist_retailsy_Information').val()<4)
        {
         var new_service_add_val=parseInt(jQuery('#exist_retailsy_Information').val())+1;
         jQuery('#exist_retailsy_Information').val(new_service_add_val);  
        }
        }
         if(split_add_more_button_split=="Add New Bann")
        {
        if(jQuery('#exist_retailsy_Banner').val()>=3)
        {
        jQuery(".customizer_banner_section_premium").show();
        return false;   
        }
         if(jQuery('#exist_retailsy_Banner').val()<3)
        {
         var new_service_add_val=parseInt(jQuery('#exist_retailsy_Banner').val())+1;
         jQuery('#exist_retailsy_Banner').val(new_service_add_val);  
        }
        }
    ...
    retailsy_theme_controls.on('click', '.social-repeater-general-control-remove-field', function () {
        ...
        if (typeof    jQuery(this).parent() !== 'undefined') {
            jQuery(this).parent().hide(500, function(){
                ...
                {
                jQuery('#exist_retailsy_Slider').val(parseInt(jQuery('#exist_retailsy_Slider').val())-1);  
                }
				if(split_delete_button_split=="Delete Infor")
                {
                jQuery('#exist_retailsy_Information').val(parseInt(jQuery('#exist_retailsy_Information').val())-1);  
                }
				if(split_delete_button_split=="Delete Banne")
                {
                jQuery('#exist_retailsy_Banner').val(parseInt(jQuery('#exist_retailsy_Banner').val())-1); 
                }
            });
        }
        return false;
    ...
    retailsy_theme_controls.on('keyup', '.customizer-repeater-title-control', function () {
        retailsy_customizer_repeater_refresh_general_control_values();
    });
    ...
    retailsy_theme_controls.on('keyup', '.customizer-repeater-subtitle-control', function () {
        retailsy_customizer_repeater_refresh_general_control_values();
    });
	
	 retailsy_theme_controls.on('keyup', '.customizer-repeater-subtitle2-control', function () {
        retailsy_customizer_repeater_refresh_general_control_values();
    });
	
	retailsy_theme_controls.on('keyup', '.customizer-repeater-subtitle3-control', function () {
        retailsy_customizer_repeater_refresh_general_control_values();
    });
	
	retailsy_theme_controls.on('keyup', '.customizer-repeater-subtitle4-control', function () {
        retailsy_customizer_repeater_refresh_general_control_values();
    });
	
	retailsy_theme_controls.on('keyup', '.customizer-repeater-subtitle5-control', function () {
        retailsy_customizer_repeater_refresh_general_control_values();
    });

    retailsy_theme_controls.on('keyup', '.customizer-repeater-shortcode-control', function () {
        retailsy_customizer_repeater_refresh_general_control_values();
    });

    retailsy_theme_controls.on('keyup', '.customizer-repeater-text-control', function () {
        retailsy_customizer_repeater_refresh_general_control_values();
    });

    retailsy_theme_controls.on('keyup', '.customizer-repeater-link-control', function () {
        retailsy_customizer_repeater_refresh_general_control_values();
    });

    retailsy_theme_controls.on('keyup', '.customizer-repeater-text2-control', function () {
        retailsy_customizer_repeater_refresh_general_control_values();
    });
	
	retailsy_theme_controls.on('keyup', '.customizer-repeater-button2-control', function () {
        retailsy_customizer_repeater_refresh_general_control_values();
    });

    retailsy_theme_controls.on('keyup', '.customizer-repeater-link2-control', function () {
        retailsy_customizer_repeater_refresh_general_control_values();
    });
	
	retailsy_theme_controls.on('change','.customizer-repeater-checkbox', function(){
    ...
    /*----------------- Socials Repeater ---------------------*/
    retailsy_theme_controls.on('click', '.social-repeater-add-social-item', function (event) {
        event.preventDefault();
        var th = jQuery(this).parent();
        var id = 'customizer-repeater-social-repeater-' + retailsy_customizer_repeater_uniqid();
        if (typeof th !== 'undefined') {
    ...
    retailsy_theme_controls.on('click', '.social-repeater-remove-social-item', function (event) {
        event.preventDefault();
        var th = jQuery(this).parent();
        var repeater = jQuery(this).parent().parent();
        th.remove();
        retailsy_customizer_repeater_refresh_social_icons(repeater);
        return false;
    });

    retailsy_theme_controls.on('keyup', '.customizer-repeater-social-repeater-link', function (event) {
        event.preventDefault();
        var repeater = jQuery(this).parent().parent();
        retailsy_customizer_repeater_refresh_social_icons(repeater);
        return false;
    ...
    retailsy_theme_controls.on('change', '.customizer-repeater-social-repeater-container .icp', function (event) {
        event.preventDefault();
...
function retailsyescapeHtml(string) {
    'use strict';
    //noinspection JSUnresolvedFunction
    string = String(string).replace(new RegExp('\r?\n', 'g'), '<br />');
    string = String(string).replace(/\\/g, '&#92;');
    return String(string).replace(/[&<>"'\/]/g, function (s) {
        return retailsyentityMap[s];
    });
