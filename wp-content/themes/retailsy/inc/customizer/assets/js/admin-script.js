( function( $ ){
    $( document ).ready( function(){
      $( '.retailsy-btn-get-started' ).on( 'click', function( e ) {
          e.preventDefault();
          $( this ).html( 'Processing.. Please wait' ).addClass( 'updating-message' );
          $.post( retailsy_ajax_object.ajax_url, { 'action' : 'install_act_plugin' }, function( response ){
        ...
        $.ajax( ajaxurl,
          {
            type: 'POST',
            data: {
              action: 'retailsy_dismissed_notice_handler',
              type: type,
            }
          } );
      } );
